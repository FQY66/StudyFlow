...
LIST_BASE = "https://www.peoplehibl.com/html/zixun/"
TOTAL_PAGES = int(os.getenv("PEOPLEHIBL_TOTAL_PAGES", "59"))

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
OUT_DIR = os.path.join(BASE_DIR, "data", "raw")
OUT_ALL_LINKS = os.path.join(OUT_DIR, "peoplehibl_all_detail_links.txt")
OUT_UNIQUE_LINKS = os.path.join(OUT_DIR, "peoplehibl_unique_detail_links.txt")
...
def list_page_url(page: int) -> str:
    # 人民研学资讯分页实际为静态路径：
    # 第1页: /html/zixun/
    # 第2页: /html/zixun/2.html
    # 第N页: /html/zixun/N.html
    if page <= 1:
        return LIST_BASE
...
def is_detail_url(url: str) -> bool:
    # 资讯详情示例: /html/2026/zixun_0428/1938.html
    path = urlparse(url).path.lower()
    return re.search(r"^/html/20\d{2}/zixun_\d{4}/\d+\.html$", path) is not None


def extract_detail_links(html: str) -> list[str]:
    soup = BeautifulSoup(html, "html.parser")

    # 与 crawler_peoplehibl.py 对齐：先移除明显噪声区块
    for sel in [
...
def main() -> None:
    ...
    for p in range(1, TOTAL_PAGES + 1):
        ...
        try:
            ...
            print(f"  - 提取到 {len(links)} 条详情链接，累计 {len(all_links)}")
        except Exception as e:
